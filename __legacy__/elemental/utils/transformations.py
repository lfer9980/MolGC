...
from elemental.utils import MatrixTransform
...
class DotsTransform(MatrixTransform):
    ...
    def __init__(self, regression: Optional[callable] = None):
        """
            ...
            :param: regression: Optional regression function for molecular plane fitting
        """
    ...
    def __call__(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        """
            ...
            :return: Aligned molecular coordinates as DataFrame with original columns
        """
        ...
        data_with_homogeneous = self._translate_to_origin(data_with_homogeneous, ref1_idx)

        # Update the coordinates in the result DataFrame
        result_df.iloc[:, 1:4] = data_with_homogeneous[:, 1:4]

        ref2_idx = self._find_next_reference_atom(data_with_homogeneous, ref1_idx, [ref1_idx])
        if ref2_idx != -1:
            self.reference_atoms.append(ref2_idx)
            data_with_homogeneous = self._align_to_x_axis(data_with_homogeneous, ref2_idx)
            result_df.iloc[:, 1:4] = data_with_homogeneous[:, 1:4]

            ref3_idx = self._find_next_reference_atom(data_with_homogeneous, ref2_idx, [ref1_idx, ref2_idx])
            if ref3_idx != -1:
                self.reference_atoms.append(ref3_idx)
                data_with_homogeneous = self._align_to_xy_plane(data_with_homogeneous, ref3_idx)
                result_df.iloc[:, 1:4] = data_with_homogeneous[:, 1:4]
    ...
    @classmethod
    def _calculate_center(cls, coordinates: np.ndarray) -> tuple[floating, floating, floating]:
        """
    ...
    @classmethod
    def _find_reference_atom_by_distance(cls, data: np.ndarray, center: Tuple[floating, floating, floating]) -> int:
        """
        ...
        for element in unique_elements:
            ...
            for i, coord in enumerate(element_coords):
                ...
                dist = np.sqrt(np.abs(x - center[0]) + np.abs(y - center[1]) + np.abs(z - center[2]))

                original_idx = np.where((data == coord).all(axis=1))[0][0]
                distances.append([dist, original_idx])

            distances.sort(key=lambda x: x[0])
            element_distances.append([len(distances), element, distances[0][1], distances[0][0]])
    ...
    @classmethod
    def _find_next_reference_atom(cls, data: np.ndarray, reference_idx: int, exclude_indices: List[int]) -> int:
        """
            Find the next reference atom based on distance from current reference.
            :param: data: Coordinate data
    ...
    def _translate_to_origin(self, data: np.ndarray, reference_idx: int) -> np.ndarray:
        ...
        for i in range(len(data)):
            ...
            transformed_coords = self.apply_transformation(coords_homogeneous, translation_matrix)
    ...
    def _align_to_x_axis(self, data: np.ndarray, target_idx: int) -> np.ndarray:
        ...
        for i in range(len(data)):
            coords_homogeneous = np.append(data[i, 1:4].astype('float64'), 1)
            transformed_coords = self.apply_transformation(coords_homogeneous, rotation_matrix)
            transformed_data[i, 1:4] = transformed_coords[:3]
    ...
    def _align_to_xy_plane(self, data: np.ndarray, target_idx: int) -> np.ndarray:
        """
            ...
            :param: data: Coordinate data
            :target_idx: Index of target atom
            :return: Translated coordinates
        """
        target_coords = data[target_idx, [1, 3]].astype('float64')
        x, z = target_coords
    ...
    def get_transformation_summary(self) -> dict:
        ...
        return {
            ...
            'num_atoms': len(self.original_data) if self.original_data is not None else 0,
            'transformation_complete': self.transformed_data is not None,
            'original_columns': list(self.original_data.columns) if self.original_data is not None else [],
            'final_columns': list(self.transformed_data.columns) if self.transformed_data is not None else []
        }
...
if __name__ == "__main__":
    def create_sample_molecule() -> pd.DataFrame:
        """
        Create a sample molecule for testing. CH4 (methane)
        """
    ...
    try:
        ...
        ax1.scatter(sample_molecule['x'], sample_molecule['y'], sample_molecule['z'], c=colors, s=100)
        ax1.set_xlabel('X')
        ax1.set_ylabel('Y')
        ax1.set_zlabel('Z')
        ...
        ax2.scatter(aligned_coords['x'], aligned_coords['y'], aligned_coords['z'], c=colors, s=100)
        ax2.set_xlabel('X')
        ax2.set_ylabel('Y')
        ax2.set_zlabel('Z')
        ...
        ax3.scatter(complex_molecule['x'], complex_molecule['y'], complex_molecule['z'], c=colors, s=100)
        ax3.set_xlabel('X')
        ax3.set_ylabel('Y')
        ax3.set_zlabel('Z')
        ...
        ax4.scatter(aligned_complex['x'], aligned_complex['y'], aligned_complex['z'], c=colors, s=100)
        ax4.set_xlabel('X')
        ax4.set_ylabel('Y')
        ax4.set_zlabel('Z')
