...
class MoleculesPlotter:
    ...
    def __call__(self, data: list[FileModel], target_idx: int):
        ...
        fig.add_trace(self._scatter_atoms(coords, f'{target.software} - {target.functional}'))
        fig.add_traces(self._draw_bonds(bonds, f'{target.software} - {target.functional}', 'red', 1))
        ...
        fig.add_trace(self._draw_text_points(px, py, pz, labels=labels, size=4,
                                             name=f'CENTER - {target.software} - {target.functional}'))

        x, y, z = coords['x'].astype(float), coords['y'].astype(float), coords['z'].astype(float)
        ...
        for i, item in enumerate(data):
            ...
            fig.add_trace(self._scatter_atoms(coords, f'{item.software} - {item.functional}'))
            fig.add_traces(self._draw_bonds(bonds, f'{item.software} - {item.functional}', 'black', 0.2))
            ...
            fig.add_trace(
                self._draw_text_points(px, py, pz, labels=labels, size=2,
    ...
    @classmethod
    def _draw_bonds(cls, bonds: pd.DataFrame, name: str, color: str = 'red', opacity: float = 1) -> list:
        """
    ...
    @staticmethod
    def _draw_text_points(px: list[float], py: list[float], pz: list[float], labels: list[str | int], size: int,
                          name: str):
        """
