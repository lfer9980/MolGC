...
class DataReport:
    ...
    def rmsd_graphs(self, data_type: str, data: list[FileModel]) -> list[dict]:
        ...
        generated_fig = px.line(
            rmsd_data, x="functional", y="RMSD", text=rmsd_data["label"], markers=True
        )
    ...
    def general(self, data: defaultdict[any, list]):
        """grouped_idx = defaultdict(list) for _, item in enumerate(self.data):
