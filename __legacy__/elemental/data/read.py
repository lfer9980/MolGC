...
from elemental.utils import read_file, read_csv
...
class DataReader:
    ...
    def __init__(self):
        ...
        self.CASTEP_REGEX = re.compile(r"x\s+([A-Z][a-z]?)\s+\d+\s+([-\d.]+)\s+([-\d.]+)\s+([-\d.]+)\s+x")
    ...
    def gaussian(self, path: str) -> pd.DataFrame:
        ...
        atoms_number: pd.DataFrame = original_df.iloc[find_name_list[0]:find_name_list[0] + self.GAUSSIAN_LINES]
        ...
        e_table['texto'] = [added]
        e_table = e_table.replace(to_replace={self.GAUSSIAN_SLASH_DOUBLE}, value={'*'}, regex=True)
        e_table = e_table['texto'].str.split(pat='*', n=1000, expand=True)

        e3_table['texto'] = [e_table.iloc[0][3]]
        e3_table = e3_table['texto'].str.split(pat=self.GAUSSIAN_SLASH, n=1000, expand=True)
    ...
    def castep(self, path: str) -> pd.DataFrame:
        ...
        start_block = re.search(r"BFGS\s*:\s*Final Configuration:\s*=+\s+", self.content)
        if not start_block:
    ...
    def fhi(self, path: str) -> pd.DataFrame:
        ...
        if not atom_lines or len(atom_lines) != number_atoms:
            sys.stdout.write(f"Expected {number_atoms} atoms, but found {len(atom_lines)}.")
            return pd.DataFrame()
