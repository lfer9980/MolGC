...
from elemental.enums import FuncEnum, FileModel, FunctionalModel
from elemental.data import DataReader, DataLoader, DataEnergy, DataReport
...
class IndividualAnalysis:
    ...
    def process_data(self):
        ...
        for variant, idx in grouped_idx.items():
            ref_idx = next(
                (
                    i for i in idx
                    if (self.data[i].software, self.data[i].functional) == self.functional
                ),
                None
            )
    ...
    def _report_by_family(self, data: defaultdict[any, list]) -> any:
        """
            ...
            :param data: a defaultdict with information grouped by specific category
        """
    ...
    def _select_functional(self):
        ...
        while True:
            try:
                user_selection = int(input('Enter the integer number of the selected reference: \n'))
        ...
        sys.stdout.write(f'Selected option: Software: {self.functional[0]}, Functional: {self.functional[1]} \n')
        return None
