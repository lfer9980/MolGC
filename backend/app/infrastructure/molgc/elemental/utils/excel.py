...
class ReadExcel:
    ...
    def group_by_family(self):
        """Group routes by family, assuming the same families in each
        functional."""
        families = set().union(*(m.keys() for m in self.file_map.values()))
        for fam in sorted(families):
            files = [
                self.file_map[func][fam]
    ...
    def process_family(cls, family: str, files: list):
        """Reads and concatenates the excels of a family, calculates T1, P1, P2
        (average MAE, normalized TOPSIS and ranking).
    ...
    def save_family_results(self, family: str, t1, p1, p2):
        ...
        fig1.update_traces(textposition="top center").update_layout(
            title=f"{family} MAE"
        )
        fig1.write_image(os.path.join(out, f"{family}_MAE.png"))

        fig2 = px.scatter(p2, x="Functional", y="Ranking", text="Ranking")
        fig2.update_traces(textposition="top center").update_layout(
            title=f"{family} TOPSIS Ranking"
        )
    ...
    def compile_global(self, all_p1: list, all_p2: list, families: list):
        ...
        """
        df_mae = pd.concat(
            [p1.set_index("Functional")["Average MAE"] for p1 in all_p1], axis=1
        )
        ...
        df_rank = pd.concat(
            [p2.set_index("Functional")["Ranking"] for p2 in all_p2], axis=1
        )
        ...
        fig_mae = px.scatter(
            df_mae.reset_index(),
            x="Functional",
            y="Overall Average MAE",
            text="Overall Average MAE",
        )
        fig_mae.update_traces(textposition="top center").write_image(
            os.path.join(out, "Overall_MAE.png")
        )
        ...
        fig_rank = px.scatter(
            ...
            x="Functional",
            y="Overall Avg Rank",
            text="Overall Avg Rank",
        )
        fig_rank.update_traces(textposition="top center").write_image(
            os.path.join(out, "Overall_TOPSIS.png")
        )


if __name__ == "__main__":
    processor = ReadExcel(main_dir="../Files", output_dir="../CrossValidationResults")
    processor()
